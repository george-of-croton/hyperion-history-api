...
async function findForksOnRange(blocks: HyperionBlock[], rpc: JsonRpc) {
  ...
  for (const currentBlock of blocks) {
    ...
    if (previousBlock.block_num !== currentBlockNumber - 1) {
      console.log(
        `\nBlock number mismatch, expected: ${currentBlockNumber - 1} got ${
          previousBlock.block_num
        }`
      );
      missingBlocks.push({
  ...
  for (const range of forkedBlocks) {
    ...
    if (args.dry) {
      ...
      if (resultAccounts.body.hits.total?.value > 0) {
        console.log(
          "[WARNING]",
          resultAccounts.body.hits.total?.value,
          "accounts needs to be updated"
        );
        deleteAccounts += resultAccounts.body.hits.total?.value;
      }
    } else {
      const indexExists = await client.indices.exists({
    ...
    if (args.dry) {
      ...
      if (resultVoters.body.hits.total?.value > 0) {
        console.log(
          "[WARNING]",
          resultVoters.body.hits.total?.value,
          "voters needs to be updated"
        );
        deleteVoters += resultVoters.body.hits.total?.value;
      }
    } else {
      const indexExists = await client.indices.exists({
    ...
    if (args.dry) {
      ...
      if (resultProposals.body.hits.total?.value > 0) {
        console.log(
          "[WARNING]",
          resultProposals.body.hits.total?.value,
          "proposals needs to be updated"
        );
        deleteProposals += resultProposals.body.hits.total?.value;
      }
    } else {
      const indexExists = await client.indices.exists({
        index: searchProposals.index,
      });
      ...
      if (indexExists.body) {
        ...
        console.log(
          `Index ${searchProposals.index} doesn't exist. Unable to delete.`
        );
      }
    }
    ...
    if (args.dry) {
      ...
      if (resultLinks.body.hits.total?.value > 0) {
        console.log(
          "[WARNING]",
          resultLinks.body.hits.total?.value,
          "links needs to be updated"
        );
        deleteLinks += resultLinks.body.hits.total?.value;
      }
    } else {
      const indexExists = await client.indices.exists({
        index: searchLinks.index,
      });
      ...
      if (indexExists.body) {
        ...
        console.log(
          `Index ${searchLinks.index} doesn't exist. Unable to delete.`
        );
      }
    }
    ...
    if (args.dry) {
      ...
      if (resultPermissions.body.hits.total?.value > 0) {
        console.log(
          "[WARNING]",
          resultPermissions.body.hits.total?.value,
          "permissions needs to be updated"
        );
        console.log({ lte: range.start, gte: range.end });
        deletePermissions += resultPermissions.body.hits.total?.value;
      }
    } else {
      const indexExists = await client.indices.exists({
        index: searchPermissions.index,
      });
      if (indexExists.body) {
    ...
    if (!range.ids) {
      console.log(
        "Invalid file format! Please check that you are calling the repair tool with the right file."
      );
      process.exit(1);
    }
